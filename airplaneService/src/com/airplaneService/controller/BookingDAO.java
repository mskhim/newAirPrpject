...
public class BookingDAO {
	private final String SELECT_SQL = "SELECT * FROM BOOKING_JOIN_ALL_VIEW ORDER BY CODE";
	private final String SELECT_BY_CODE_SQL = "SELECT * FROM BOOKING_JOIN_ALL_VIEW WHERE trim(CODE) like TRIM(?)";
	private final String SELECT_BY_CUSTOMER_SQL = "SELECT * FROM BOOKING_JOIN_ALL_VIEW WHERE CUSTOMER_NO = ? ";
	    ...
	    try {
	        ...
	        while (rs.next()) {
	            ...
	            if (currentBvo == null || !currentBvo.getCode().equals(code)) {
	                ...
	                seatList.add(seat);
	                currentBvo = new BookingVO(code, bookingDate, name, depCountry, arvCountry, price, departureHour, arrivalHour, planeName, seatList);
	            } else {
	                // 같은 그룹이라면 seat만 추가
	                seatList.add(seat);
	            }
	...
	// code가 들어있는 BookingVO를 받아서 그에 해당하는 db의 BookingVO를 반환
		...
		try {
			...
			while (rs.next()) {
				...
				Timestamp bookingDate = rs.getTimestamp("BOOKING_DATE");
				seatList.add(seat);
				bvo = new BookingVO(code,  bookingDate, name, depCountry, arvCountry, price, departureHour, arrivalHour, planeName,seatList);
			}
